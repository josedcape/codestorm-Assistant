...
const AgentSelector: React.FC<AgentSelectorProps> = ({ 
  ...
  return (
    <div className="agent-selector-container futuristic-container w-full">
      <h3 className="gold-accent mb-3 font-semibold">Agentes Especializados</h3>
      
      <div className="space-y-3">
        {agents.map((agent) => (
          <div 
            key={agent.id}
            className={`p-3 rounded-lg border silver-border transition-all duration-300 relative ${
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {currentAgent === agent.id && (
                <Badge 
                  className="bg-amber-500 hover:bg-amber-500 text-xs font-normal"
                  variant="outline"
                >
                  Activo
                </Badge>
